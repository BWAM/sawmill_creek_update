...
pcode <-
  read.csv(
    "L:/BWAM Share/SMAS/data/cleaned_files/Final_Chemistry_ITS/MASTER_S_CHEM_PARAMETER_2023-01-10.csv"
  )
))
...
colnames_ref <-
  read.csv(here::here("data/col_names/stayCALM_colnames.csv"),
...
colnames_smas_chem <- function(df) {
  ...
  names(chem.good)[match(col.vec.short[, "chem_db_col"], names(chem.good))] =
    col.vec.short[, "stayCALM_col"] #match the column names
  ...
  df1 <-
    data.frame(matrix(vector(), ncol = max(length(sc_all_cols))))#creates an empty data frame
...
colnames_smas_insitu <- function(df) {
  ...
  names(insitu.good)[match(col.vec.short[, "insitu_db_col"], names(insitu.good))] =
    col.vec.short[, "stayCALM_col"] #match the column names
...
param.names <-
  read.csv(here::here("data/col_names/stayCALM_parameters.csv"))

#fix the DO fraction
all.chemistry <- all.chemistry %>%
  mutate(
    fraction = case_when(
      parameter == "DISSOLVED OXYGEN" & units == "mg/l" ~ "dissolved",
      parameter == "PH" &
        units == "ph units" ~ "total",
...
all.chemistry <- all.chemistry %>%
  mutate(stayCALM_parameter = case_when(
    stayCALM_parameter == "" ~ paste(parameter),
    is.na(stayCALM_parameter) ~ paste(parameter),
    TRUE ~ paste(stayCALM_parameter)
  ))

#make them all lowercase
all.chemistry$stayCALM_parameter <-
  tolower(all.chemistry$stayCALM_parameter)
...
all.chemistry$sample_id <-
  paste(all.chemistry$site_id, all.chemistry$date_text, sep = "_")
...
violations <- violations %>%
  ...
  mutate(exceed = case_when(attaining_wqs == "no" ~ 1,
                            TRUE ~ 0)) %>%
